...
1) Log-transform of target: We modeled targetRevenue = log(∑revenue + 1), as provided, to stabilize variance and reduce skewness, which improves linear-model assumptions and reduces the impact of outliers.

2) Constant/NZV column removal + PCA de-noising: We removed constant/near-zero-variance predictors and applied PCA (93% variance retained) to create stable inputs for PLS/PCR/SVR/GAM, reducing multicollinearity and improving generalization.

--->

### Modeling

```{r table1_final_simplified, echo=FALSE, message=FALSE, warning=FALSE}
suppressPackageStartupMessages({
  library(data.table)
  library(dplyr)
  library(caret)
...
<!---
Resampling: We used 5-fold cross-validation (stratified by the regression target via caret::createFolds) and generated out-of-fold (OOF) predictions for each model to obtain unbiased CV estimates. We stacked models using OOF predictions only.

Our final predictions are a stacked ensemble of Ridge, Elastic Net, PLS, PCR, SVR (raw & PCA), GAM-on-PCA, and a two-stage (Logit×SVR) model. We learned non-negative blend weights with NNLS on OOF predictions. This blends the complementary strengths of linear bias (Ridge/PLS/PCR) and smooth nonlinearity (SVR/GAM), plus the purchase propensity separation.

We captured nonlinearity via SVR (RBF) and GAM smooths over PCA components. PCA also mitigated multicollinearity and noise before feeding PLS/PCR/SVR/GAM.


We believe our secret sauce was log-transform of revenue, which helped us stabilize the variance. Constant/NZV pruning + PCA to de-noise and improve generalization was highly impactful as well. We also implemented robust imputation and sanitization to prevent failure in SVR/GAM while keeping the pipeline deterministic.


We encountered:

PCA on constant columns → fixed by NZV removal before PCA.

NA/Inf errors in GAM/SVR → fixed with a universal sanitize_df() wrapper on all inputs.

pls::pcr predict dimension inconsistency → normalized via as.numeric().
...
knitr::purl(input = "Sozudogru_HW7.Rmd", output = "ReportHW7.R",documentation = 0)
